...
class SimulationController:
    ...
    def run(self) -> None:
        ...
        while self.simulation_status.run:
            ...
            self.fire_controller.spread_fire(self.model, self.simulation_status)    
            self.view_controller.update()
        pygame.quit()

    def resolve_event(self, event) -> None:
        if event.type == pygame.QUIT:
            self.simulation_status.run = False
        if not self.simulation_status.animation_started:
            if pygame.mouse.get_pressed()[0]:  # LEFT
                ...
                if row < self.model.cells_y and col < self.model.cells_x:
                    ...
                    x, y = pos
                    self.model.wind_direction = self.button_handler.click_proper_button(x,y)
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    self.simulation_status.animation_started = True
